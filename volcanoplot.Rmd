...
author: "Maria Doyle", "Liz Christie"
...
```{r}
goi <- read_tsv("https://zenodo.org/record/2529117/files/volcano_genes?download=1")
```
...
Let's make the volcano plot and label this custom set of genes.
...
```{r}
de_results <- mutate(de_results, signif_f=factor(signif, levels=c("Up", "Not signif", "Down")))
ggplot(de_results, aes(logFC, -log10(P.Value), col=signif)) +
  geom_point() +
