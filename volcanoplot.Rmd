...
If we want all the points to be the same size we don't put size= inside `aes()`. However if we wanted to map the size of the points to a column in our dataset then we would put it inside `aes()`. 
...
It's gene symbols stored in a column. We need to convert these symbols out of the column format. We can do that with dplyr's `pull()`. We give `pull()` the `goi` object and the name of the column we want to convert. We'll store the symbols in an object called `goi_syms`.
...
Like before we need to extract the gene symbols from the `top10` info and have them not in a column. We'll store them in an object called `top10_syms`.
...
Let's label these genes in the volcano plot.
...
Again we could use `geom_text_repel()` to stop the labels overlapping. But we'll continue now without the gene labels because `geom_text_repel()` can be slow on the server.
...
ggplot(de_results, aes(logFC, -log10(P.Value), col=signif)) +
  geom_point() +
  scale_colour_manual(name="My new name", values=c("Up"="red", "Not signif"="grey", "Down"="blue"), breaks=c("Up", "Down", "Not signif"))
```
...
- We can use dplyr's `pull()` to extract a set of values from a column
