...
Next we load in our RNA-seq data. We will use the published data for 12 samples from mouse luminal and basal cells (ref). The file we will use is tab-separated, so we will use the `read_tsv()` function from the tidyverse readr package to read it in. We will store the contents of our file in an object called `de_results`.
...
To make our volcano plot we make a scatterplot using `geom_point()`. We plot logFC on the x axis and -log10(P.Value) on the y axis.
...
Let's colour points that are significant at adj.P.Value < 0.05. To do this we use dplyr's `mutate()` to add a column called "sig" and we use `ifelse()` to say if our adj.P.Val is below 0.05 we add "Sig" to the sig column, othewise add "Not sig". 
...
We can also make the plot look nicer, for example, adding a title and changing the background. First let's store our inital plot in an object called `p`, to make it easier to see what we're changing.
