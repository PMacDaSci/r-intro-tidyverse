...
There are different geoms we can use to create different types of plot e.g. bar plot versus box plot, to see some of the geoms available see the ggplot2 reference here: https://ggplot2.tidyverse.org/reference/ and the ggplot2 cheat sheet here: https://www.rstudio.com/wp-content/uploads/2016/11/ggplot2-cheatsheet-2.1.pdf

An example scatter plot
To demonstrate how ggplot works we will use one of the built-in datasets that R has. We will use the dataset called `women` that contains the average heights and weights for American women.
...
Let's make a scatterplot. Note that to add a geom to the plot we use the `+` operator. This must go at the end of the line, and not at the beginning or ggplot2 will give an error.
...
We can use the bioinformatician's good friend Google to try to figure out why. Through Google we can find an answer on Stack Overflow (Stack Overflow is another great friend of bioinformaticians) https://stackoverflow.com/questions/49593634/unintended-line-across-x-axis-of-density-plot-r. It is because `geom_density()` plots a polygon shape. If we don't want that line we can adapt the code given in that Stack Overflow post.
...
We can also make box plots to visualise the distribution of counts for each sample. With box plots we can see the distribution of counts for each sample, the distributions don't overlap as in the density plot.
...
Note that we get a warning here about rows containing non-finite values being removed. This is because some of the genes have zero counts. We could add a small number as before to each count but the box plot ends up with no bottom whisker as there are so many zero counts. Usually we would filter lowly expressed genes before creating the box plots. We will plot the data as is, as we are just demonstrating the different types of plots that can be made here.
...
* Make as many plots as you can from bar plots, density plots, boxplots and violin plots using the normalised counts (some starter code is below)
* Colour by CellType and then by Status where appropriate.
