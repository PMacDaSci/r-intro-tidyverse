...
If we would like to colour the lines in the plot instead of the area we can use `colour=` instead of `fill=`.
...
If we don't want the line at the bottom (polygon) we can use `geom_line()` e.g. https://stackoverflow.com/questions/49593634/unintended-line-across-x-axis-of-density-plot-r
```{r}
ggplot(data=counts, mapping=aes(x=log2(Counts+2), y = stat(density), colour=Sample)) +
...
# Exercise: Colour by Status instead of CellType
