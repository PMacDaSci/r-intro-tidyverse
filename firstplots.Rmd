...
In R we use `<-` to assign _values_ to _objects_. To create an object, we need to give it a name followed by the assignment operator `<-`, and the value we want to give it.
...
the left. So, after executing above the value of `counts` is the contents of the counts file. For historical reasons, you can also use `=`
...
Objects can be given any name such as `x`, `rnaseq_counts` but some reommendations on naming objects are:

  * You want your object names to be explicit and not too long. 
  * They cannot start with a number (`2x` is not valid, but `x2` is). 
  * R is case sensitive
(e.g., `counts` is different from `Counts`). 
...
In the Environment Tab on the top right in RStudio we can also see the number of rows and columns in the objects we have in our session.
...
Before we start plotting the RNA-seq data we'll demonstrate how ggplot works using one of the built-in datasets in R (you can see the built-in datasets by typing `data()`. We will use the dataset called `women` that contains the average heights and weights for American women.

Frist take a look at the dataset.
...
We can use the bioinformatician's good friend Google to try to figure out why. Through Google we can find an answer on Stack Overflow (Stack Overflow is another great friend of bioinformaticians) https://stackoverflow.com/questions/39679057/r-ggplot2-stat-count-must-not-be-used-with-a-y-aesthetic-error-in-bar-graph/39679104. As pointed out in that post, we could specify y=Counts if we had already calculated the total counts for each sample, and we were just plotting the calculated value. However, our input data contains the individual counts for each gene for each sample we want to **sum** the counts for each sample. If we look at the help for `geom_bar()` (or that post):
