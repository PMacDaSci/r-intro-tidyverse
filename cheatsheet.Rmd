...
**`filter()`**  
choosing rows (from dplyr)

**`gather()`**  
function that enables converting from wide to long (tidy) format (from dplyr)

**`geom_text_repel()`**  
geom to add add text non-overlapping labels (from ggrepel)
...
**`pull()`**  
extract values (from dplyr)

**`read_tsv()`**  
read a tab-separated file into R (from readr)

**`select()`**  
choosing columns (from dplyr)
...
**function**   
pre-defined commands, can be loaded in from packages or user-created
...
**vector**  
a data structure in R containing a collection of items of the same data type e.g c("TP53", "BRCA1")
...
\   
**`$`**  
access columns e.g. `counts$GENENAME`  
  
\   
**`%in%`**  
function used to test if a value is in a set of values  
